...
.bbd2-bg{position:absolute;inset:0;z-index:0;background:radial-gradient(ellipse 62% 50% at 50% 42%,rgba(125,150,200,.08),transparent 62%),radial-gradient(ellipse 100% 80% at 50% 118%,rgba(45,55,90,.18),transparent 60%),#030407}
.bbd2-stage{position:absolute;inset:6% 4.5%}
.bbd2-title,.bbd2-slice{position:absolute;display:grid;place-items:center;pointer-events:none;white-space:nowrap;font-weight:700;font-size:clamp(26px,10cqw,190px);letter-spacing:-.035em;line-height:1}
.bbd2-title{inset:0;z-index:1;opacity:0;animation:bbd2-cracks-in 1.4s cubic-bezier(.16,1,.3,1) forwards}
.bbd2-title--under span,.bbd2-title--under .bbd2-stack{color:rgba(188,198,220,.24)}
.bbd2--portrait .bbd2-title,.bbd2--portrait .bbd2-slice{font-size:clamp(44px,20cqw,190px)}
.bbd2-stack{display:flex;flex-direction:column;align-items:center;gap:.08em;transform:rotate(-8deg);line-height:.92}
.bbd2-stack em{font-style:normal;display:block}
.bbd2-stack em:nth-child(2){font-size:.44em;align-self:flex-end;margin-right:8%;opacity:.85}
.bbd2-cracks{position:absolute;inset:0;width:100%;height:100%;z-index:2;pointer-events:none;opacity:0;animation:bbd2-cracks-in 1.4s cubic-bezier(.16,1,.3,1) forwards}
@keyframes bbd2-cracks-in{to{opacity:1}}
.bbd2-cracks path{fill:none;vector-effect:non-scaling-stroke}
.bbd2-cracks-line path{stroke:rgba(198,214,244,.34);stroke-width:1}
.bbd2-cracks-glow path{stroke:rgba(170,195,240,.10);stroke-width:2.6}
.bbd2-cracks-fine path{stroke:rgba(198,214,244,.17);stroke-width:.75}
.bbd2-pane{position:absolute;inset:0;z-index:5;transform-style:preserve-3d;pointer-events:none}
.bbd2-shard{position:absolute;transform-origin:50% 50%;pointer-events:auto;will-change:transform;transition:filter .45s cubic-bezier(.16,1,.3,1)}
.bbd2-shard--hot{z-index:40!important;filter:brightness(1.22) drop-shadow(0 42px 54px rgba(0,0,0,.72)) drop-shadow(0 0 30px rgba(150,185,255,.18))}
...
.bbd2-glassimg::after{content:'';position:absolute;inset:0;background:linear-gradient(132deg,rgba(165,185,232,.13) 0%,rgba(165,185,232,.03) 30%,transparent 46%,transparent 60%,rgba(112,96,178,.08) 100%);mix-blend-mode:screen}
.bbd2-slice>span,.bbd2-slice>.bbd2-stack{color:rgba(222,232,252,.52);mix-blend-mode:screen;filter:blur(.3px);transform:var(--jt);text-shadow:0 0 12px rgba(165,195,250,.22)}
.bbd2-slice>.bbd2-stack{transform:var(--jt) rotate(-8deg)}
.bbd2-specular{position:absolute;inset:0;opacity:0;background:radial-gradient(42% 42% at var(--mx,50%) var(--my,50%),rgba(205,225,255,.34),rgba(140,170,230,.08) 46%,transparent 72%);mix-blend-mode:screen;transition:opacity .5s cubic-bezier(.16,1,.3,1)}
.bbd2-shard--hot .bbd2-specular{opacity:1}
.bbd2-sound{position:absolute;right:16px;bottom:12px;z-index:50;appearance:none;border:none;background:none;padding:6px;cursor:pointer;transition:filter .3s cubic-bezier(.16,1,.3,1),transform .3s cubic-bezier(.16,1,.3,1)}
.bbd2-sound img{display:block;width:74px;height:auto;pointer-events:none;animation:bbd2-fade-in .5s ease forwards}
@keyframes bbd2-fade-in{from{opacity:0}to{opacity:1}}
.bbd2-sound:hover,.bbd2-sound:focus-visible{filter:brightness(1.35) drop-shadow(0 6px 14px rgba(0,0,0,.55));transform:translateY(-2px);outline:none}
.bbd2-sound:active{transform:translateY(0) scale(.96)}
.bbd2-sound[aria-pressed='false']{filter:brightness(.8)}
.bbd2-sound[aria-pressed='false']:hover,.bbd2-sound[aria-pressed='false']:focus-visible{filter:brightness(1.1) drop-shadow(0 6px 14px rgba(0,0,0,.55))}
.bbd2-credit{position:absolute;left:18px;bottom:18px;z-index:50;font-family:ui-monospace,'SF Mono',Menlo,monospace;font-size:10px;letter-spacing:.08em;color:rgba(214,224,244,.85);text-decoration:none;opacity:.16;transition:opacity .35s}
.bbd2-credit:hover,.bbd2-credit:focus-visible{opacity:.8;outline:none}
@media(prefers-reduced-motion:reduce){.bbd2-shard,.bbd2-specular,.bbd2-sound,.bbd2-credit{transition:none}.bbd2-cracks,.bbd2-title{animation:none;opacity:1}}
`;
...
function spriteStyle(setKey, id) {
...
function baseOf(id, seed) {
  ...
  return {
    rx: (r(1) - 0.5) * 3.4, ry: (r(2) - 0.5) * 4.2, tz: r(3) * 16,
    px: 0, py: 0, sc: 1,
    ...BASE_POSE[id],
  };
}
...
export default function GlassHero({
  ...
  useEffect(() => {
    ...
    Promise.all(
      ...
      if (!cancelled) { clearTimeout(timeout); setReady(true); }
    });
  ...
  const titleNode = portrait ? (
    <span className="bbd2-stack">
      {title.split(' ').map((w, i) => (<em key={i}>{w}</em>))}
    </span>
  ) : (<span>{title}</span>);
  ...
  useEffect(() => {
    ...
    const shards = Array.from(root.querySelectorAll('[data-shard]'));
    ...
    const cur = shards.map((_, i) => baseOf(P[i].id, i + 1));
    const tgt = shards.map((_, i) => baseOf(P[i].id, i + 1));
    ...
    const tick = () => {
      ...
      for (let i = 0; i < shards.length; i++) {
        ...
        c.rx += (t.rx - c.rx) * k; c.ry += (t.ry - c.ry) * k;
        c.tz += (t.tz - c.tz) * k; c.px += (t.px - c.px) * k;
        c.py += (t.py - c.py) * k; c.sc += (t.sc - c.sc) * k;
    ...
    const onRootLeave = () => {
      globalX = 0; globalY = 0;
      tgt.forEach((t, i) => { if (!hovered.has(i)) Object.assign(t, baseOf(P[i].id, i + 1)); });
      wake();
    };
  ...
  return (
    <>
      ...
      <section
        ref={rootRef}
        className={`bbd2 ${portrait ? 'bbd2--portrait' : ''} ${ready ? 'bbd2--ready' : ''}`}
        style={{ height }}
        aria-label={title}
      >
        ...
        <div className="bbd2-stage">
          {ready && (
            <>
              <div className="bbd2-title bbd2-title--under" aria-hidden="true">{titleNode}</div>
              <svg className="bbd2-cracks" viewBox={`0 0 ${cracks.w} ${cracks.h}`} preserveAspectRatio="none" aria-hidden="true">
                <g className="bbd2-cracks-glow">{cracks.main.map((d, i) => <path key={i} d={d} />)}</g>
                <g className="bbd2-cracks-line">{cracks.main.map((d, i) => <path key={i} d={d} />)}</g>
                <g className="bbd2-cracks-fine">{cracks.fine.map((d, i) => <path key={i} d={d} />)}</g>
              </svg>
              <div className="bbd2-pane" aria-hidden="true">
                {pieces.map((p, i) => {
                  ...
                  return (
                    <div key={p.id} data-shard className="bbd2-shard" style={{ left: `${p.x}%`, top: `${p.y}%`, width: `${p.w}%`, height: `${p.h}%`, zIndex: 10 + (2 - p.ring) }}>
                      <div className="bbd2-inlay" style={{
                        WebkitMaskImage: `url(${atlasUrl})`, maskImage: `url(${atlasUrl})`,
                        WebkitMaskSize: sprite.backgroundSize, maskSize: sprite.backgroundSize,
                        WebkitMaskPosition: sprite.backgroundPosition, maskPosition: sprite.backgroundPosition,
                      }}>
                        <div className="bbd2-glassimg" style={{ backgroundImage: `url(${atlasUrl})`, backgroundSize: sprite.backgroundSize, backgroundPosition: sprite.backgroundPosition }} />
                        <div className="bbd2-slice" style={{
                          width: `${10000 / p.w}%`, height: `${10000 / p.h}%`,
        ...
        <button type="button" className="bbd2-sound" aria-pressed={soundOn} aria-label={soundOn ? 'Mute sound' : 'Enable sound'}
          onClick={() => { setSoundOn(v => !v); try { audioRef.current ??= new AudioContext(); } catch {} }}>
          {ready && <img src={`${assetsBase}/${soundOn ? 'sound-on' : 'sound-off'}.png`} alt="" draggable={false} />}
        </button>
      </section>
    </>
  );
